...
├── view_queries_sql/
│   ├── README.md
│   ├── queries_customer_behavior.sql
│   ├── queries_demographic_analysis.sql
│   ├── queries_product_performance.sql
│   └── queries_geo_analysis.sql
│
├── python/
│   ├── 02_load_to_mysql.py
...
Para demostrar que la anonimización preserva el valor analítico de los datos, se escribieron más de 40 queries sobre las cuatro vistas de propósito limitado. Están organizadas en `view_queries_sql/` y cubren:

- Análisis de valor de vida del cliente, retención y recencia
- Desglose de ingresos por país, género y rango de edad
- Rendimiento por categoría de producto y método de pago
- Tendencias regionales y crecimiento de ingresos mes a mes

Las técnicas SQL incluyen window functions (`ROW_NUMBER`, `RANK`, `LAG`),
CTEs, agregación condicional y cálculos basados en fechas.
...
| Queries analíticas sobre vistas anonimizadas | 40+ |
